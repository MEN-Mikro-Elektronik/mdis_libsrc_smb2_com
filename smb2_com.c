 ...
 *
 * $Log: smb2_com.c,v $
 * Revision 1.20  2013/11/06 08:14:46  dpfeuffer
 * R:1. Windows: BSOD during shutdown at SC26 with SMB drivers
 * M:1.a) smbComExit: synchronize with SMB transfer that is in progress
 *     b) all external SMB functions: check for initialized smbHdl added
 *
...
{
	int32 error = 0;
	SMB_COM_HANDLE *smbComHdl;
	DBGCMD(	static const char functionName[] = "SMB2 - smbComExit"; )

	if( smbHdlP == NULL )
		return 0;

	smbComHdl = (SMB_COM_HANDLE*)((u_int8*)smbHdlP + sizeof(SMB_HANDLE));

	DBGWRT_1( (DBH, "%s smbHdlP %08p\n", functionName, smbHdlP ) );

	/*
	 * Get semaphore to wait for a SMB transfer that might be already in progress
	...
	case ERR_SUCCESS:
		...
		break;
	case ERR_OSS_TIMEOUT:
		DBGWRT_ERR( (DBH, "*** WARNING %s: OSS_SemWait timeout\n", functionName) );
		break;
	default:
		DBGWRT_ERR( (DBH, "*** %s: OSS_SemWait error=0x%x\n", functionName, error) );
	}
